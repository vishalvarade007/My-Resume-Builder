...
function MyResume() {
    ...
    return (
        <div className={selectedTemplate !== ""?'container w-100 overflow-scroll':'container w-100 overflow-hidden'}>
        <div  className=' row mt-2 p-5'>
            <div className='w-100 d-flex justify-content-center'>
                {selectedTemplate !== ""?<Link to="/detailsfillingpage/keyskills">
                    <button className='btn btn-primary me-4 p-2'> Go-Back</button>
                </Link>:<Link to="/">
                    <button className='btn btn-primary me-4 p-2'> Go-Back</button>
                </Link>}
                {selectedTemplate !== ""?<button className='btn btn-success ms-3 p-2'onClick={downloadComponentPDF}>
                    Save Resume
                </button>:null}
            </div>
        </div>
        <div  className='  mt-2 p-5 w-100 ' style={selectedTemplate !== ""?{ minWidth:"1200px"}:{ minWidth:"1200px", overflow:'hidden'}}>
            <div className=' w-100  d-flex justify-content-center '>
                <div className='w-100 ' >
                    <div id='divToPrint' className='w-100'>
                        {/* In this div, user selected template is rendered alongwith the details filled by the user. */}
                        {selectedTemplate===""
                        ?<div><h1 style={{color:'red'}}>Please select</h1><h1 style={{color:'red'}}>a template!</h1></div>
